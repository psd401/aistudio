...
function applyNavigationStringFields(
  ...
  if (
    data.type !== undefined &&
    (data.type === "link" || data.type === "section" || data.type === "page")
  )
...
function applyNavigationRelationFields(
  ...
  if (data.capabilityId !== undefined)
    updateData.capabilityId = data.capabilityId ?? null;
  if (data.requiresRole !== undefined)
    updateData.requiresRole = data.requiresRole ?? null;
}
...
export async function getNavigationItemsAction(): Promise<
  ActionState<SelectNavigationItem[]>
> {
  ...
  try {
    ...
    return handleError(
      error,
      "Failed to get navigation items. Please try again or contact support.",
      {
...
function navigationItemCreateData(
  data: InsertNavigationItem,
): Parameters<typeof createNavigationItem>[0] {
  return {
    label: data.label,
    icon: data.icon,
    ...
    type: data.type || "page",
    parentId: parseOptionalNumericField("parentId", data.parentId),
    capabilityId: parseOptionalNumericField("capabilityId", data.capabilityId),
...
export async function createNavigationItemAction(
  ...
  try {
    ...
    if (!(await hasRole("administrator"))) {
      log.warn("Non-admin attempted to create a navigation item", {
        userId: session.sub,
      });
    ...
    const newItem = await createNavigationItem(navigationItemCreateData(data));
    ...
    return handleError(
      error,
      "Failed to create navigation item. Please try again or contact support.",
      {
...
export async function updateNavigationItemAction(
  ...
  try {
    ...
    if (!(await hasRole("administrator"))) {
      log.warn("Non-admin attempted to update a navigation item", {
        userId: session.sub,
      });
      throw ErrorFactories.authzInsufficientPermissions("administrator");
    }
    ...
    return handleError(
      error,
      "Failed to update navigation item. Please try again or contact support.",
      {
...
export async function deleteNavigationItemAction(
  ...
  try {
    ...
    // (REV-COR-039).
    if (!(await hasRole("administrator"))) {
      log.warn("Non-admin attempted to delete a navigation item", {
        userId: session.sub,
      });
      throw ErrorFactories.authzInsufficientPermissions("administrator");
    }
    ...
    return handleError(
      error,
      "Failed to delete navigation item. Please try again or contact support.",
      {
