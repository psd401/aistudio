...
class SmokeAdapter implements Adapter {
  ...
  async findByUid(uid: string): Promise<AdapterPayload | undefined> {
    for (const [key, record] of SmokeAdapter.records) {
      if (key.startsWith(`${this.model}:`) && record.payload.uid === uid) {
...
class CookieJar {
  private readonly values = new Map<string, { value: string; path: string }>();

  capture(response: Response, requestUrl: string): void {
    const requestPath = new URL(requestUrl).pathname;
    const lastSlash = requestPath.lastIndexOf("/");
    const defaultPath = lastSlash <= 0 ? "/" : requestPath.slice(0, lastSlash);
    ...
    for (const cookie of response.headers.getSetCookie()) {
      ...
      const path = cookie.match(/(?:^|;)\s*path=([^;]+)/i)?.[1] ?? defaultPath;
      if (value.length === 0 || /(?:^|;)\s*max-age=0(?:;|$)/i.test(cookie)) {
...
const nativeRedirectUri = "org.psd401.atrium-capture:/oauth/callback";
const thirdPartyClientId = "third-party-browser";
const thirdPartyRedirectUri = "https://third-party.example/oauth/callback";
...
async function reserveOrigin(): Promise<string> {
...
function createSmokeProvider(origin: string): Provider {
  ...
  return new Provider(origin, {
    ...
    interactions: {
      ...
      url: (_ctx, interaction) => `/oauth/authorize?uid=${interaction.uid}`,
    },
...
async function handleProviderRequest(
  ...
  if (path.startsWith("/oauth/authorize/interaction/")) {
    const interaction = await provider.interactionDetails(request, response);
    await finishInteraction(provider, request, response, interaction);
    return;
  }
...
class SmokeAuthorizationFlow {
  constructor(
    private readonly provider: Provider,
    private readonly origin: string,
    private readonly observedPrompts: ObservedPrompt[],
  ) {}

  async authorizationCode(expire = false): Promise<string> {
    const client = await this.provider.Client.find(clientId);
    if (!client) throw new Error("Smoke OAuth client not found");
    ...
    const code = new this.provider.AuthorizationCode({
      accountId: "1",
      client,
      grantId,
  ...
  authorizationUrl(options: {
    ...
    return `${this.origin}/auth?${params.toString()}`;
  }
  ...
  async authorize(options: {
    ...
    let current = this.authorizationUrl(options);
    const promptStart = this.observedPrompts.length;
  ...
  async assertAuthorizationRejected(
    url: string,
...
async function assertAuthorizationPrompts(
  flow: SmokeAuthorizationFlow,
  observedPrompts: ObservedPrompt[],
): Promise<void> {
  const browserSession = new CookieJar();
  const signedOutBrowser = await flow.authorize({
    clientId,
    redirectUri,
  ...
  const signedInBrowser = await flow.authorize({
    clientId,
    redirectUri,
  ...
  const nativeFirstParty = await flow.authorize({
...
async function assertAuthorizationRejections(
  ...
  await flow.assertAuthorizationRejected(
  ...
  assert.equal(nativeClient.redirectUriAllowed(nativeRedirectUri), true);
  assert.equal(
    nativeClient.redirectUriAllowed("org.psd401.atrium-capture:/oauth/wrong"),
    false,
  );
  assert.equal(
    loopbackClient.redirectUriAllowed("http://127.0.0.1:49152/oauth/callback"),
    true,
  );
  assert.equal(
    loopbackClient.redirectUriAllowed("http://localhost:49152/oauth/callback"),
    false,
  );
  assert.equal(
    loopbackClient.redirectUriAllowed("http://127.0.0.1:49152/wrong"),
    false,
  );
}
...
async function assertTokenRejections(
  ...
  const wrongVerifierCode = await flow.authorizationCode();
  ...
  const wrongRedirectCode = await flow.authorizationCode();
  ...
  const expiredCode = await flow.authorizationCode(true);
  ...
  assert.equal(browserFakeCode.allowOrigin, ATRIUM_CAPTURE_EXTENSION_ORIGIN);
  ...
  assert.equal(browserFakeRefresh.allowOrigin, ATRIUM_CAPTURE_EXTENSION_ORIGIN);
...
async function assertTokenLifecycle(
  flow: SmokeAuthorizationFlow,
  origin: string,
): Promise<void> {
  const code = await flow.authorizationCode();
  ...
  assert.equal(
    decodeProtectedHeader(token.body.access_token).kid,
    "smoke-active",
  );
  ...
  const refreshCode = await flow.authorizationCode();
  ...
  assert.equal(refreshSeed.status, 200, JSON.stringify(refreshSeed.body));
...
async function assertRevocation(
  flow: SmokeAuthorizationFlow,
  origin: string,
): Promise<void> {
  const revokeCode = await flow.authorizationCode();
  ...
  assert.equal(await accessAdapter.find(revokeJti), undefined);
  ...
  assert.equal(browserRevocation.allowOrigin, ATRIUM_CAPTURE_EXTENSION_ORIGIN);
}

async function main(): Promise<void> {
  const origin = await reserveOrigin();
  const provider = createSmokeProvider(origin);
  const observedPrompts: ObservedPrompt[] = [];
  const server = await startProviderServer(provider, origin, observedPrompts);
  const flow = new SmokeAuthorizationFlow(provider, origin, observedPrompts);

  try {
    await assertAuthorizationPrompts(flow, observedPrompts);
    await assertAuthorizationRejections(flow, provider);
    await assertTokenRejections(flow, origin);
    await assertTokenLifecycle(flow, origin);
    await assertRevocation(flow, origin);
