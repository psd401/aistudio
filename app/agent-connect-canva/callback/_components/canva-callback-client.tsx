...
import { handleCanvaCallback, type CanvaCallbackResult } from "@/actions/agent-canva.actions"

export function CanvaCallbackClient() {
  ...
  const paramError = oauthError
    ? `Canva reported: ${oauthError}. Please try the link again.`
    : !code || !state
      ? "Missing authorization code. Ask your agent for a new link."
      : null

  const [result, setResult] = useState<CanvaCallbackResult | null>(null)

  useEffect(() => {
    if (paramError || !code || !state) return
    let cancelled = false
    void (async () => {
      const response = await handleCanvaCallback(code, state)
      if (cancelled) return
  ...
  return (
    <div className="flex min-h-screen items-center justify-center p-4">
      <div className="w-full max-w-md rounded-lg border bg-card p-8 shadow-sm text-center">
        {pending && <p className="text-muted-foreground">Connecting your Canva account...</p>}
        {success && (
          <>
            <h1 className="mb-2 text-xl font-semibold">Canva connected ✅</h1>
            <p className="text-muted-foreground">
              Your agent can now act on your Canva account. You can close this tab and
              return to chat.
            </p>
          </>
        )}
