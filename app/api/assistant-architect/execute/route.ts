...
export async function POST(req: Request) {
  ...
  try {
    ...
    // retrieval / repository tools, so the previous String(architect.userId)
    // (a numeric id) never matched and silently disabled owner-repository
    // access on non-owner executions. Only look it up when the executor is not
    // the owner — owner === executor is already covered by userCognitoSub =
    // session.sub. getUserById throws if the owner row is gone (e.g. deleted
    // user) — treat that as no owner sub rather than failing the execution.
