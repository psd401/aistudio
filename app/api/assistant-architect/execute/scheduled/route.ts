            ...
            try {
              ...
              await executeQuery(
                (db) => db.execute(sql`
                  ...
                  VALUES (${context.executionId}, ${prompt.id}, ${sql.raw(`$json$${inputDataJson}$json$::jsonb`)}, ${text || ''}, ${resultStatus}, ${startedAt.toISOString()}::timestamp, ${completedAt.toISOString()}::timestamp, ${executionTimeMs})
                `),
                'savePromptResult'
              );
