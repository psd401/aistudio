...
function mcpUrl(): URL {
  ...
  if (base.protocol !== "https:" && !localHttp) {
    throw ErrorFactories.sysConfigurationError("APP_BASE_URL must use HTTPS");
  }
...
function isToolsCallParams(value: unknown): boolean {
  if (!isObjectRecord(value)) return false;
  if (typeof value.name !== "string") return false;
  if (!/^[a-z][a-z0-9_]{0,127}$/.test(value.name)) return false;
  if (!isObjectRecord(value.arguments)) return false;
  return Object.keys(value).every(
    (key) => key === "name" || key === "arguments",
  );
}

function isValidBody(value: unknown): value is AistudioBrokerBody {
  if (!isObjectRecord(value)) return false;
  const body = value;
  ...
  if (!isObjectRecord(body.params)) return false;
  return body.method !== "tools/call" || isToolsCallParams(body.params);
}
...
function aistudioToolTimeout(body: {
  method: string;
  params?: unknown;
}): number {
  if (body.method !== "tools/call") return 180_000;
...
export async function POST(request: NextRequest) {
  ...
  if (!isValidBody(body)) {
    return NextResponse.json(
      { error: "Invalid AI Studio operation" },
      { status: 400 },
    );
  }
