...
type ConnectorLookup = NonNullable<
  ReturnType<typeof useConnectorToolsOptional>
>["getConnectorInfo"];

function toolNameOf(part: object): string | null {
  if (!("toolName" in part) || typeof part.toolName !== "string") return null;
  return part.toolName;
}

function classifyToolGroup(
  toolCalls: object[],
  getConnectorInfo: ConnectorLookup | undefined,
): GroupType {
  const names = toolCalls.map(toolNameOf);
  if (
    names.every((name) => name !== null && DIRECT_RENDER_TOOLS.includes(name))
  ) {
    return "direct";
  }
  if (!getConnectorInfo) return "generic";
  const connectorFlags = names.map(
    (name) => name !== null && Boolean(getConnectorInfo(name)),
  );
  if (connectorFlags.every(Boolean)) return "connector";
  if (connectorFlags.some(Boolean)) return "mixed";
  return "generic";
}
...
function GenericToolCard({
  ...
  onToggle,
  toolCount,
}: PropsWithChildren<Omit<ToolCardProps, "label">>) {
  return (
...
export function ToolGroup({
  ...
  endIndex,
  children,
}: PropsWithChildren<ToolGroupProps>) {
  ...
  const groupType = useMemo(
    () => classifyToolGroup(toolCalls, getConnectorInfo),
