...
<!-- openwiki: broken internal link [app-features/overview.md#artifact-data-query-transport-1788] heading anchor "artifact-data-query-transport-1788" does not exist in "app-features/overview.md". Fix the href or restore the target, then delete this comment. -->
14. **Artifact Data Concurrency** (#1788): Queries now run in parallel (up to 6 at once) via fetch Route Handler, not serialized Server Actions. Excess requests queue rather than reject. The server's 30s budget spans preflight + MCP handshake + execution — see **[app-features/overview.md](app-features/overview.md#artifact-data-query-transport-1788)** for concurrency, timeout, and queue contracts.
...
<!-- openwiki: broken internal link [app-features/overview.md#preview-frame-preservation-1788] heading anchor "preview-frame-preservation-1788" does not exist in "app-features/overview.md". Fix the href or restore the target, then delete this comment. -->
19. **Artifact Preview Frame** (#1788): The canvas keeps the preview iframe mounted on tab toggle to avoid re-running queries. A frame is kept hidden, never first created hidden — charting libraries would initialize at zero size. See **[app-features/overview.md](app-features/overview.md#preview-frame-preservation-1788)** for mount key contract.
