...
class MockFormData {
  ...
  append(name: string, value: string | File, _filename?: string) {
...
jest.mock('@/lib/document-processing', () => ({
  extractTextFromDocument: jest.fn().mockImplementation((buffer: Buffer, _fileType: string) => {
...
describe('File Validation', () => {
  ...
  it('should reject files over 25MB', async () => {
    const largeContent = Array.from({length: 26 * 1024 * 1024}).fill('a').join('');
