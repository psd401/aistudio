...
const mockGetChainPrompts = jest.fn<
  () => Promise<Array<{ id: number; repositoryIds?: number[] }>>
>(() => Promise.resolve([]))
const mockCreateChainPrompt = jest.fn<(data: unknown) => Promise<unknown>>()
const mockUpdateChainPrompt = jest.fn<(id: number, data: unknown) => Promise<unknown>>()
const mockGetAccessibleRepositoryIds = jest.fn<
...
const mockValidateAssistantRepositoryAudience = jest.fn<
  () => Promise<AudienceValidationResult>
>(() => Promise.resolve({ isCompatible: true, mismatches: [] }))
const mockValidateAssistantRepositoryAudienceForRepositoryIds = jest.fn<
  (
    assistantId: number,
...
const mockExecuteQuery = jest.fn<
  (_fn: unknown, label?: string) => Promise<unknown[]>
>(
...
jest.mock('@/lib/assistant-architect/repository-audience', () => ({
  validateAssistantRepositoryAudience: mockValidateAssistantRepositoryAudience,
  validateAssistantRepositoryAudienceForRepositoryIds:
    mockValidateAssistantRepositoryAudienceForRepositoryIds,
}))
...
describe('assistant-architect mutation authorization', () => {
  ...
  it('addChainPromptAction rejects a repository ID the owner cannot access', async () => {
    mockGetAssistantArchitectById.mockResolvedValue({ id: 5, userId: 1, status: 'draft' })
    mockGetAccessibleRepositoryIds.mockResolvedValue([10])

    const res = await mod.addChainPromptAction('5', {
      ...promptData,
      repositoryIds: [10, 11],
    })
    ...
    expect(mockGetAccessibleRepositoryIds).toHaveBeenCalledWith([10, 11], 1)
    expect(mockCreateChainPrompt).not.toHaveBeenCalled()
  })

  it('addChainPromptAction persists bindings only when every repository is accessible', async () => {
    mockGetAssistantArchitectById.mockResolvedValue({ id: 5, userId: 1, status: 'draft' })
    mockCreateChainPrompt.mockResolvedValue({ id: 10 })
    ...
    expect(res.isSuccess).toBe(true)
    expect(mockGetAccessibleRepositoryIds).toHaveBeenCalledWith([10, 11], 1)
    expect(mockCreateChainPrompt).toHaveBeenCalledWith(
      expect.objectContaining({ repositoryIds: [10, 11] })
    )
  })
  ...
  it('blocks adding a private binding to an unrestricted approved assistant', async () => {
    ...
    expect(res.isSuccess).toBe(false)
    expect(res.message).toContain('approved assistant')
    expect(
      mockValidateAssistantRepositoryAudienceForRepositoryIds
    ).toHaveBeenCalledWith(5, [10, 11])
    expect(mockCreateChainPrompt).not.toHaveBeenCalled()
  })

  it('allows a compatible repository binding on an approved assistant', async () => {
    mockGetAssistantArchitectById.mockResolvedValue({
      id: 5,
      userId: 1,
      status: 'approved',
    ...
    mockCreateChainPrompt.mockResolvedValue({ id: 20 })
  ...
  it('addChainPromptAction rejects an accessible ephemeral repository', async () => {
    mockGetAssistantArchitectById.mockResolvedValue({ id: 5, userId: 1, status: 'draft' })
  ...
  it('updatePromptAction rejects inaccessible repository bindings', async () => {
    mockExecuteQuery.mockImplementation((_fn: unknown, label?: string) => {
      if (label === 'getChainPromptById') {
        return Promise.resolve([{
          id: 20,
          assistantArchitectId: 5,
          modelId: 1,
          enabledTools: [],
        }])
      }
    ...
    const res = await mod.updatePromptAction('20', {
      repositoryIds: [10, 11],
    })
  ...
  it('blocks a wider-role repository update on an approved assistant', async () => {
    ...
    mockGetChainPrompts.mockResolvedValue([
      { id: 20, repositoryIds: [10] },
      { id: 21, repositoryIds: [11] },
    ])
    mockValidateAssistantRepositoryAudienceForRepositoryIds.mockResolvedValue({
      isCompatible: false,
      mismatches: [{
    ...
    expect(
      mockValidateAssistantRepositoryAudienceForRepositoryIds
    ).toHaveBeenCalledWith(5, [11, 12])
    expect(mockUpdateChainPrompt).not.toHaveBeenCalled()
  })

  it('persists a compatible candidate binding update on an approved assistant', async () => {
    mockExecuteQuery.mockImplementation((_fn: unknown, label?: string) => {
      if (label === 'getChainPromptById') {
        return Promise.resolve([{
          id: 20,
    ...
    mockUpdateChainPrompt.mockResolvedValue({
      id: 20,
      repositoryIds: [12],
    })
    ...
    expect(mockUpdateChainPrompt).toHaveBeenCalledWith(20, {
      repositoryIds: [12],
    })
  })

  it('blocks submission when a bound repository has a narrower audience', async () => {
    mockGetAssistantArchitectById.mockResolvedValue({
      id: 5,
      userId: 1,
      name: 'Assistant',
    ...
    mockGetChainPrompts.mockResolvedValue([{ id: 20 }])
    mockValidateAssistantRepositoryAudience.mockResolvedValue({
      isCompatible: false,
      mismatches: [{
        repositoryId: 11,
    ...
    const res = await mod.submitAssistantArchitectForApprovalAction('5')
  ...
  it('rechecks audience compatibility before admin approval', async () => {
    ...
    const res = await mod.approveAssistantArchitectAction('5')
