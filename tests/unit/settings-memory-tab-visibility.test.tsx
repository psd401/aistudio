...
jest.mock(
  "@/app/(protected)/settings/_components/profile-tab",
  () => ({ ProfileTab: () => <div>Profile content</div> }),
)
jest.mock(
  "@/app/(protected)/settings/_components/api-keys-tab",
  () => ({ ApiKeysTab: () => <div>API key content</div> }),
)
jest.mock(
  "@/app/(protected)/settings/_components/preferences-tab",
  () => ({ PreferencesTab: () => <div>Preference content</div> }),
)
jest.mock(
  "@/app/(protected)/settings/_components/memory-tab",
...
describe("Settings memory capability visibility", () => {
  it("renders the Memory tab for a capability-granted user", () => {
    render(
      <SettingsClient {...BASE_PROPS} hasMemoryCapability />,
    )
