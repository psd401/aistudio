...
describe('Execution Results Download Utility Functions', () => {
  describe('Markdown Generation', () => {
    ...
    it('should generate complete markdown structure', () => {
      ...
      expect(markdown).toContain('Generated by AI Studio - Your Organization')
      expect(markdown).toContain('View online: https://test.example.com/execution-results/')
    })
...
function generateMarkdownHelper(result: ReturnType<typeof createMockExecutionResult>, appUrl = 'https://test.example.com'): string {
  const executedDate = new Date(result.executedAt)
  const statusEmoji = result.status === 'success' ? '✓' : result.status === 'failed' ? '✗' : '⏳'
  const duration = formatDurationHelper(result.executionDurationMs)

  const resultData = result.resultData
  const inputData = result.inputData
  const scheduleConfig = result.scheduleConfig

  // Sanitize user-controlled content
  const safeScheduleName = sanitizeMarkdownContent(result.scheduleName)
  const safeScheduleDescription = sanitizeMarkdownContent(getScheduleDescriptionHelper(scheduleConfig as Record<string, unknown>))
  ...
  if (result.status === 'success' && resultData) {
    if (typeof resultData === 'object' && resultData != null) {
      if ('content' in resultData) {
        markdown += sanitizeMarkdownContent(String(resultData.content))
      } else if ('text' in resultData) {
        markdown += sanitizeMarkdownContent(String(resultData.text))
      } else if ('output' in resultData) {
        markdown += sanitizeMarkdownContent(String(resultData.output))
      } else {
...
---
Generated by AI Studio - Your Organization${appUrl ? `\nView online: ${appUrl}/execution-results/${result.id}` : ''}
`

  return markdown
}

function generateFilenameHelper(result: ReturnType<typeof createMockExecutionResult>): string {
  const executedDate = new Date(result.executedAt)
