...
-- state between edit sessions: the y-websocket-protocol collab server
-- (lib/content/collab/collab-server.ts) hydrates a Y.Doc from `y_state` on
-- first connection and persists the encoded state back here, debounced, on change.
-- Cross-instance fan-out across ECS tasks runs through Redis; this table is the
-- source of truth on cold load and the input to immutable version snapshots.
--
-- `markdown` is a best-effort projection set on initial seed only; human-typing
-- persists do NOT re-derive it from the Y.Doc. The authoritative markdown for
-- snapshots comes from the editor client at snapshot time.
-- `revision` is a monotonic persist counter, used as an optimistic-concurrency
-- token by the agent bridge.
--
...
CREATE TABLE IF NOT EXISTS atrium_doc_state (
  ...
  -- Best-effort markdown projection; set on seed, not re-derived on every persist.
