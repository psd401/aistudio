...
const env: EnvironmentVariables = {
  ...
  BRANDING_ORG_NAME: process.env.BRANDING_ORG_NAME,
  BRANDING_APP_NAME: process.env.BRANDING_APP_NAME,
};
...
function generateMarkdownAttachment(executionResult: ExecutionResult, scheduleName: string): string {
  ...
  // Sanitize operator-configured branding values (consistent with HTML/text paths)
  const orgName = sanitizeEmailContent(env.BRANDING_ORG_NAME || 'Your Organization', 100);
  const appName = sanitizeEmailContent(env.BRANDING_APP_NAME || 'AI Studio', 100);

  markdown += `---\n\n`;
  markdown += `*Generated by ${orgName} ${appName}*\n`;
