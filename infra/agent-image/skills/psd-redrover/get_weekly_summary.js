...
function emptyWeeklySummary(vacancies, weekRange) {
  const summary = {
...
function addVacancy(summary, vacancy) {
  const dayOfWeek = DAY_FORMATTER.format(new Date(vacancy.start));
  const isFilled = !!vacancy.substitute;
  if (Object.prototype.hasOwnProperty.call(summary.by_day, dayOfWeek)) {
    ...
    summary.daily_details[dayOfWeek][isFilled ? 'filled' : 'unfilled']++;
  }
  summary[isFilled ? 'filled' : 'unfilled']++;
  incrementCount(summary.by_school, vacancy.location?.name || 'Unknown');
  incrementCount(
    summary.by_reason,
    vacancy.absenceDetail?.reasons?.[0]?.name || 'Unknown'
  );
  incrementCount(
    summary.by_position_type,
    vacancy.position?.positionType?.name || 'Unknown'
  );
}

function finalizeWeeklySummary(summary) {
...
function addWeeklyTrends(summary) {
...
function buildWeeklySummary(vacancies, weekRange) {
  const summary = emptyWeeklySummary(vacancies, weekRange);
  for (const vacancy of vacancies) addVacancy(summary, vacancy);
  // Intentionally divide by 5 so cross-week comparisons share a denominator.
  finalizeWeeklySummary(summary);
  addWeeklyTrends(summary);
  return summary;
}
