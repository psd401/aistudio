WITH stu AS (SELECT 'iReady Reading' AS meas, studentid, "window", AVG(percentile) AS s
  FROM iready_reading_diagnostics WHERE yearid=35 AND grade_level=3 AND "window" IN ('Fall','Spring') GROUP BY 2,3
  UNION ALL
SELECT 'iReady Math' AS meas, studentid, "window", AVG(percentile) AS s
  FROM iready_math_diagnostics WHERE yearid=35 AND grade_level=3 AND "window" IN ('Fall','Spring') GROUP BY 2,3
  UNION ALL
SELECT 'D:ORF Accuracy', studentid, "window", AVG(score)
  FROM dibels_scores WHERE yearid=35 AND grade_level=3 AND assessment_group='ORF Accuracy' AND "window" IN ('Fall','Spring') GROUP BY 2,3),
...
q AS (SELECT mm.*,
      ...
      FROM mm),
cls AS (SELECT meas, COALESCE(q_cls::text,'All') AS qt, ROUND(AVG(e-b) FILTER (WHERE sectionid=274382),1) AS a1, COUNT(*) FILTER (WHERE sectionid=274382) AS n1, ROUND(AVG(e-b) FILTER (WHERE sectionid=274392),1) AS a2, COUNT(*) FILTER (WHERE sectionid=274392) AS n2, ROUND(AVG(e-b) FILTER (WHERE sectionid=274423),1) AS a3, COUNT(*) FILTER (WHERE sectionid=274423) AS n3
