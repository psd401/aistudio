WITH stu AS (SELECT 'iReady Reading' AS meas, studentid, "window", AVG(percentile) AS s
  FROM iready_reading_diagnostics WHERE yearid=35 AND grade_level=4 AND "window" IN ('Fall','Spring') GROUP BY 2,3
  UNION ALL
SELECT 'iReady Math' AS meas, studentid, "window", AVG(percentile) AS s
  FROM iready_math_diagnostics WHERE yearid=35 AND grade_level=4 AND "window" IN ('Fall','Spring') GROUP BY 2,3
  UNION ALL
SELECT 'D:ORF Accuracy', studentid, "window", AVG(score)
  FROM dibels_scores WHERE yearid=35 AND grade_level=4 AND assessment_group='ORF Accuracy' AND "window" IN ('Fall','Spring') GROUP BY 2,3),
