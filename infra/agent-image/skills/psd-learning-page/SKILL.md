...
(`agnt_<uniqname>@psd401.net`, Reader is enough). On a `drive.file` 404 / consent error the
skill surfaces `psd-workspace`'s "share it with my agent account" guidance verbatim (error
`gdoc_denied`) — relay that; never tell the user to share it with their own address.
...
7. **Publish** → `psd-atrium create-artifact` + `publish --destination intranet`. Emits the
   artifact (id/slug/version) and the reader URL.
...
- **Success (exit 0):** JSON — dry-run: `{ status, mode:"dry-run", outPath, modalities,
  omissions, a11y }`; published: `{ status, mode:"published", artifact, publish, readerUrl,
  modalities, a11y }`. Relay the reader URL.
- **Errors:** `bad_args` (1) · `ingest_failed` / `gdoc_denied` / `publish_failed` (2) ·
  `a11y_violations` (3, the assembled page failed the WCAG 2.2 AA gate — fix and retry;
  contrast/reflow are browser-verified, not in this gate).
