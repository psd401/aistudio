...
function collectPolicyStatements(templateToInspect: Template): PolicyStatement[] {
  const statements: PolicyStatement[] = [];
  const policies = templateToInspect.findResources("AWS::IAM::Policy");
  ...
  const roles = templateToInspect.findResources("AWS::IAM::Role");
...
function collectWildcardActions(statements: PolicyStatement[]): string[] {
...
test("grants only the documented wildcard APIs", () => {
  expect(collectWildcardActions(collectPolicyStatements(template))).toEqual([
