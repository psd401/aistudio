...
A tier is a preference, not a requirement — candidate resolution sweeps `[tier, medium, light, high]` — so a deployment with no configured or accessible medium model still lands on the light model it used before. When that happens the reason codes also carry `workspace_artifact_min_tier_unmet`, so the presence of `workspace_artifact_min_tier` is never mistaken for proof the floor took effect. The floor is inert for the specialist `image` and `web-search` intents, whose candidates come from the specialist lists and never consult a tier: `workspace_artifact_min_tier` still records that the decision was raised, but `workspace_artifact_min_tier_unmet` is deliberately not emitted for them, because there was no tier-driven selection to miss.
