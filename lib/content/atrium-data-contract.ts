...
export const ATRIUM_DATA_AUTHORING_GUIDANCE =
  ...
  "FAILURES ARE TYPED: wrap EVERY call in try/catch and branch on `err.code`, which is one of `unauthenticated`, `forbidden`, `not_query_mode`, `rate_limited` (with `err.retryAfterSeconds`), `timeout`, `query_error`, `too_many_requests`, `unavailable`. Render a no-access/sign-in state ONLY for `forbidden` and `unauthenticated`. For `query_error` the SQL ITSELF is wrong (a bad column or syntax) — show `err.message`, which for someone who can edit the artifact is the database's own message; do NOT dress a broken query up as a permissions problem. Never assume a call succeeded, and never leave a chart silently empty on a rejection. " +
