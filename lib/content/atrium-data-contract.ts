...
export const ATRIUM_DATA_AUTHORING_GUIDANCE =
  ...
  // #1792: the two numbers here used to be literals in prose. They are now
  // interpolated from the module the parent bridge enforces them from, and the
  // outstanding ceiling (the only one that REFUSES rather than queues) is named
  // alongside them rather than left for a model to discover at runtime.
  `BUDGET: aim for 3-8 aggregate queries per page load. Fire them together (\`Promise.all\`) — they run in parallel, up to ${ARTIFACT_MAX_CONCURRENT_DATA_REQUESTS} at a time, and anything beyond that queues rather than failing; only past ${ARTIFACT_MAX_PENDING_DATA_REQUESTS} outstanding at once does a call reject with \`too_many_requests\`. The hard limit is ${ARTIFACT_QUERY_RATE_LIMIT} queries per minute per viewer per artifact; exceeding it rejects with \`rate_limited\` and \`err.retryAfterSeconds\`. So do NOT query in a loop over rows, and do NOT re-query on every filter change if one aggregate could be filtered in JavaScript. ` +
