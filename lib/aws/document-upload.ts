...
export async function generatePresignedUrl(jobId: string, fileName: string): Promise<PresignedUploadConfig> {
  try {
    ...
    throw new Error(`Failed to generate presigned URL: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
  }
}
...
export async function generateMultipartUrls(jobId: string, fileName: string, partCount: number): Promise<PresignedUploadConfig> {
  try {
    ...
    throw new Error(`Failed to generate multipart URLs: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
  }
}
...
export async function getSignedDownloadUrl(s3Key: string, expiresIn: number = 3600): Promise<string> {
  try {
    ...
    throw new Error(`Failed to generate download URL: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
  }
}