...
function validatedSkillSummary(raw: unknown): string {
  if (
    typeof raw !== "string" ||
    raw.trim().length === 0 ||
...
function validatedSkillMarkdown(raw: unknown): Uint8Array {
  const content = decodeBase64(raw, "SKILL.md", MAX_SKILL_MD_BYTES);
  const text = Buffer.from(content).toString("utf8");
  const hasFrontmatter =
    text.startsWith("---") &&
    text.slice(3).includes("---") &&
    /\nname:\s*\S/.test(text) &&
    /\nsummary:\s*\S/.test(text);
  if (!hasFrontmatter) {
...
function decodedSkillFiles(
  raw: unknown,
  skillMarkdown: Uint8Array,
): SkillFile[] {
  const rawFiles = raw === undefined ? [] : raw;
...
export class AgentSkillsService {
  ...
  async author(
    ...
    const summary = validatedSkillSummary(input.summary);
    const skillMd = validatedSkillMarkdown(input.skillMdBase64);
    const files = decodedSkillFiles(input.files, skillMd);
